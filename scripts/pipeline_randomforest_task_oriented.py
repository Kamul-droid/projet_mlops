...
def preprocess_data():
    ...
    X_train.to_csv(r"data/X_train_clean.csv", index=False)
    X_test.to_csv(r"data/X_test_clean.csv", index=False)
    pd.DataFrame(y_train).to_csv(r"data/y_train.csv", index=False)
    pd.DataFrame(y_test).to_csv(r"data/y_test.csv", index=False)
...
def train_and_log_model():
    """Entraîne le modèle Random Forest et enregistre les métriques dans MLflow."""
    mlflow.set_tracking_uri("mlflow_run")  # Remplacez par le chemin réel
    experiment_name = "experience_rf"
    ...
    # Initialiser le modèle Random Forest avec des hyperparamètres spécifiques
    ...
    logger = get_run_logger()
    logger.info(
        f"Modèle Random Forest entraîné avec succès : Accuracy={accuracy}, F1 Score={f1}, Precision={precision}, Recall={recall}, AUC-ROC={auc_roc}, Log Loss={logloss}"
    )
