...
def preprocess_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    ...
    num_pipeline = Pipeline(steps=[("imputer", SimpleImputer()), ("normalization", MinMaxScaler())])

    cat_pipeline = Pipeline(
        steps=[("imputer", SimpleImputer(strategy="most_frequent")), ("encoder", OneHotEncoder(drop="first"))]
    )

    ord_pipeline = Pipeline(steps=[("imputer", SimpleImputer(strategy="most_frequent")), ("encoder", OrdinalEncoder())])
    ...
    def __init__(self):
        ...
        self.cat_pipeline = Pipeline(
            [
                ...
                ),  # Remplit les valeurs manquantes avec la valeur la plus fréquente
                (
                    "encoder",
                    OneHotEncoder(drop="first"),
                ),  # Applique un encodage one-hot et retire la première catégorie pour éviter la colinéarité
            ]
        )
